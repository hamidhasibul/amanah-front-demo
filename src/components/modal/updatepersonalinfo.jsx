...
export const Upersonal = () => {
  ...
  const [pordaType, setPordaType] = useState("");
  const [dari, setDari] = useState("");
  const [mazhub, setMazhub] = useState("");
  ...
  function profilesubmit() {
    ...
    fetch(`${import.meta.env.VITE_SERVER}/updatepersonalinfo`, {
      method: "POST",
      body: data,
    })
      .then((res) => res.json())
      .then((res) => {
        if (res.message) {
          toast.success("ব্যাক্তিগত তথ্য আপডেট করা হয়েছে", {
  ...
  const getAllInfo = () => {
    ...
    fetch(`${import.meta.env.VITE_SERVER}/getbiodata`, {
      ...
      .then((res) => res.json())
      .then((res) => {
        setActiveid(res.personalinfo[0].userid);
        setDob(res.personalinfo[0].dob);
        setHeight(res.personalinfo[0].height);
        setWeight(res.personalinfo[0].weight);
        setColor(res.personalinfo[0].color);
        setMstatus(res.personalinfo[0].mstatus);
        setBloodgroup(res.personalinfo[0].bloodgroup);
        setDistrict(res.personalinfo[0].district);
        setUpazila(res.personalinfo[0].upazila);
        setPresentaddress(res.personalinfo[0].presentaddress);
        setPordaType(res.personalinfo[0].pordaType);
        setDari(res.personalinfo[0].dari);
        setMazhub(res.personalinfo[0].mazhub);
        setPolitics(res.personalinfo[0].politics);
        setSelfinfo(res.personalinfo[0].selfinfo);
      })
      .catch((err) => console.log(err));
  };
  ...
  return (
    <>
      <div
        ...
        aria-hidden="true"
      >
        <div class="modal-dialog passmodal_doc">
          <div class="modal-content font-lal passmodal_docc">
            ...
            <div class="modal-body row font-lal px-3">
              <div className="col-4">
                <div class="mb-3">
                  <label
                    for="dateofbirth"
                    class="form-label text-muted font-14"
                  >
                    জন্মসাল
                  </label>
                  <input
                ...
                <div className="col-6">
                  <div class="mb-3">
                    ...
                    <select
                      ...
                      aria-label="Default select example"
                    >
                      <option selected>{pordaType}</option>
                      <option>ওরনা </option>
                      <option>হিজাব </option>
                      <option>নিকাব </option>
                    </select>
