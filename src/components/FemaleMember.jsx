...
export const FemaleMember = ({ allBiodata, calcAge, searchFilter }) => {
  ...
  const [veilFilter, setVeilFilter] = useState("");
  ...
  const pageCount = Math.ceil(
    allBiodata?.filter((item) => {
      if (
        ...
        (!veilFilter ||
          item.pordaType.toLocaleString().includes(veilFilter.toLocaleString()))
      ) {
        return item;
      }
          ...
          ?.filter((item) => {
            if (
              ...
              (!veilFilter ||
                item.pordaType
                  .toLocaleString()
                  .includes(veilFilter.toLocaleString()))
            ) {
              return item;
            }
          })
